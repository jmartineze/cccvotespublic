...
<div class="px-4 py-5 max-w-2xl mx-auto">
    <div class="mb-5">
        ...
        <h1 class="font-display text-xl font-800" style="color: var(--color-text);">Edit Submission</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.submissions.update', $submission) }}" enctype="multipart/form-data"
        ...
        <div>
            ...
            <select name="contest_id" class="input {{ $errors->has('contest_id') ? 'input-error' : '' }}"
                    x-ref="contestSelect" x-on:change="updateScenario($event.target)" required>
                @foreach($contests as $contest)
                    <option value="{{ $contest->id }}" data-type="{{ $contest->contest_type }}" {{ old('contest_id', $submission->contest_id) == $contest->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="text" name="discord_user" class="input {{ $errors->has('discord_user') ? 'input-error' : '' }}"
                value="{{ old('discord_user', $submission->discord_user) }}" placeholder="username#0000" required>
        ...
        <div>
            ...
            <input type="text" name="character_name" class="input {{ $errors->has('character_name') ? 'input-error' : '' }}"
                value="{{ old('character_name', $submission->character_name) }}" placeholder="Sakura Hana" required>
        ...
        <div>
            ...
            <input type="text" name="country" class="input {{ $errors->has('country') ? 'input-error' : '' }}"
                value="{{ old('country', $submission->country) }}" placeholder="Japan" required>
            ...
            <div>
                ...
                <select name="gender" class="input {{ $errors->has('gender') ? 'input-error' : '' }}" required>
                    @foreach(['Male', 'Female', 'Trans'] as $g)
                        <option value="{{ $g }}" {{ old('gender', $submission->gender) === $g ? 'selected' : '' }}>{{ $g }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="style" class="input {{ $errors->has('style') ? 'input-error' : '' }}" required>
                    @foreach(['Anime', 'Realistic'] as $s)
                        <option value="{{ $s }}" {{ old('style', $submission->style) === $s ? 'selected' : '' }}>{{ $s }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <textarea name="backstory" class="input {{ $errors->has('backstory') ? 'input-error' : '' }}"
                ...
                x-init="backstoryLen = $el.value.length">{{ old('backstory', $submission->backstory) }}</textarea>
        ...
        <div x-show="showScenario" x-cloak>
            ...
            <textarea name="scenario_description" class="input {{ $errors->has('scenario_description') ? 'input-error' : '' }}"
                ...
                :required="showScenario">{{ old('scenario_description', $submission->scenario_description) }}</textarea>
        ...
        <div>
            <label class="section-label block mb-2">
                ...
                <span class="ml-2 font-mono text-xs" style="color: var(--color-muted);">{{ $submission->images->count() }}/12 — click ✕ to mark for removal</span>
            </label>
            <div class="grid grid-cols-3 gap-2">
                @foreach($submission->images as $i => $image)
                <div class="relative aspect-square rounded-lg overflow-hidden"
                     style="background: var(--color-faint);"
                     x-data="{ marked: false }">
                    @if($image->isVideo())
                        <video src="{{ $image->url }}" class="w-full h-full object-cover transition-opacity" :class="marked ? 'opacity-30' : ''" muted playsinline preload="metadata"></video>
                        <div class="absolute bottom-1 left-1 px-1 rounded text-xs font-mono" style="background: rgba(0,0,0,0.7); color: #00d4ff;">▶</div>
                    @else
                        <img src="{{ $image->url }}" class="w-full h-full object-cover transition-opacity" :class="marked ? 'opacity-30' : ''">
                    @endif
                    <div class="absolute top-1 left-1 w-5 h-5 rounded-full flex items-center justify-center text-xs font-mono font-700"
                         style="background: rgba(0,0,0,0.7); color: white;">{{ $i + 1 }}</div>
                    <input type="checkbox" name="delete_images[]" value="{{ $image->id }}"
                           x-ref="chk" class="hidden" :checked="marked">
                    <button type="button"
                            x-on:click="marked = !marked; $refs.chk.checked = marked"
                            class="absolute top-1 right-1 w-5 h-5 rounded-full flex items-center justify-center transition-colors"
                            :style="marked ? 'background: rgba(100,100,100,0.9)' : 'background: rgba(239,68,68,0.85)'"
                            style="color: white; font-size: 0.7rem;"
                            :title="marked ? 'Undo' : 'Remove'">
                        <span x-text="marked ? '↩' : '✕'"></span>
                    </button>
                    <div x-show="marked" class="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <span class="text-xs font-mono font-700" style="color: #f87171;">Remove</span>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        @if($submission->images->count() < 12)
        <div>
            <label class="section-label block mb-1.5">
                Add Images / Videos
                <span class="ml-2 font-mono text-xs" style="color: var(--color-muted);">up to {{ 12 - $submission->images->count() }} more · max 10MB each</span>
            </label>
...
function submissionForm() {
    return {
        ...
        addFiles(newFiles) {
            const remaining = {{ 12 - $submission->images->count() }} - this.files.length;
